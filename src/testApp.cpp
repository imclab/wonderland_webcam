...
void testApp::draw()
{
    ...
    for(int a = 0; a < threshNum; a++)
    {
        ...
        {
            ofEllipse(cvContour[a].blobs[i].centroid.x, cvContour[a].blobs[i].centroid.y, 3, 10);
        }
...
void testApp::imageUpdate()
{
    ...
    if (vidGrabber.isFrameNew())
    {
        ...
        for(int i = 0; i < threshNum; i++)
        {
            ...
            cvContour[i].findContours(cvGrIm[i], (1280*720)/10000, (1280*720)/50, 4, true,false);
        }
...
void testApp::applyLaplacian()
{
    ...
    {
        cvGrIm[i] = cvIm;
    //    cv::GaussianBlur(cvGrIm[i].getPixelsRef(), cvGrIm[i].getPixelsRef(), cv::Size(3,3), 0.0);
        cvContour[i].findContours(cvGrIm[i], (1280*720)/10000, (1280*720)/50, 7, true,false);
    }
}
